...
let sq x = x * x
let calculateZInterception (z : Zombie) (x : int) (y : int) (h : Human) =
                                let ox = float(z.X - x)
                                let oy = float(z.Y - y)
...
//                                                                   int(x1) + z.X, int(y1) + z.Y
                                           ...
                                           | true -> x, y
                                           | false -> let toTravel = toKillSpot - float(killDist - 100)
                                                      let r = toTravel / toKillSpot
                                                      let x1 = float(tx - x) * r
                                                      let y1 = float(ty - y) * r
                                                      Math.Max(int(x1) + x, 0), Math.Max(int(y1) + y, 0)
...
//                                                        (zth - float(400)) / float(zWalkDist) > (ath - float(1600)) / float(walkDist))
    ...
    let cantSave = zombieToHuman
                        |> Array.filter (fun zh ->  let z, h, zth = zh
                                                    let xi, yi = calculateZInterception z x y h
    ...
    let tX, tY = match zombieToHuman.Length > 0 with
                    | true -> let zombie, human, zth = zombieToHuman |> Array.minBy (fun zh -> let z, h, zth = zh
                              ...
                              calculateZInterception zombie x y human
                              //human.X, human.Y
...
//                                | false -> zombie.NextX, zombie.NextY
                    | false ->  Console.Error.WriteLine("Holding posish")

                                let zomb = zombies |> Array.minBy (fun z -> calculateDist x y z.X z.Y)
